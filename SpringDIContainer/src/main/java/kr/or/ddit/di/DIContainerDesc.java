 ...
 * 
 * Container ?? 객체(bean)의 생명주기 관리자, 객체의 생성(소멸), 필요 객체의 주입구조를 통한 의존관계 형성
 * 
 * 사용단계
 ...
 * 		- GenericXmlpplicationContext - resource loader 활용할 수 있는 prefix를 통해 자원의 위치 결정
 * 4. 필요 객체(bean) 주입 - getBean()
 * 
 * 컨테이너 빈 관리 정책
 * 1. 특별한 설정(scope)이 없는 한 등록된 모든 빈은 싱글톤으로 관리
 * 		singleton : 빈 하나당 객체가 하나씩 생성 및 관리. 주입 횟수와 상관없이 하나의 객체가 공유
 * 		prototype : 주입될 때 마다 새로운 객체가 생성 및 관리됨. 객체의 생성 시점이 지연됨
 * 		scope="singleton" -> 4번 주입 객체 1번 생성
 * 		scope="prototype" -> 4번 주입 객체 4번 생성
 * 		request/session(web용 scope) : requestScope, sessionScope의 생명주기와 동일해짐
 * 2. 특별한 설정(lazy-init)이 없는 한 컨테이너가 초기화될때 등록된 모든 객체를 일시에 생성 (무겁게 동작)
...
//		// 싱글톤 활용중 - container를 통해 대신 하는 구조
