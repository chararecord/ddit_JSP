...
import static java.util.Calendar.*; // 캘린더가 가지고 있는 모든 static member는 다 여기 들어와있음
...
public class CalendarWrapper {
	...
	// wrapper가 adapter를 반드시 하나만 받을 이유는 없음
	public CalendarWrapper(Calendar adaptee, Locale locale) {
		...
		dayOfWeekFirst = adaptee.get(DAY_OF_WEEK); // 목요일
		offset = dayOfWeekFirst - SUNDAY; // 목요일-일요일 : 그 사이 일수
		lastDate = adaptee.getActualMaximum(DAY_OF_MONTH); // 마지막날
		
		currentYear = adaptee.get(YEAR);
		currentMonth = adaptee.get(MONTH);
		
		adaptee.add(MONTH, -1); // 전달 22년 11월
		beforeYear = adaptee.get(YEAR);
		beforeMonth = adaptee.get(MONTH);
		
		adaptee.add(MONTH, +2); // 다음달 23년 1월
		nextYear = adaptee.get(YEAR);
		nextMonth = adaptee.get(MONTH);
		
		adaptee.add(MONTH, -1); // 이번달로 돌려놓기 22년 12월
	}
